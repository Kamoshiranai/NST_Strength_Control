...
def resize_to_max_size(image: Image, max_size: int, multiple_of_8: bool = False, multiple_of_16: bool = False):
...
def preprocess_image(max_image_size: int, multiple_of_16 = False):
    ...
    prep_resize = lambda x: prep(resize_to_max_size(x, max_image_size, multiple_of_16))
    return prep_resize

def preprocess_greyscale_image(max_image_size: int, multiple_of_16 = False):
    prep = transforms.Compose([
        transforms.Lambda(lambda x: resize_to_max_size(x, max_image_size, multiple_of_16)), 
        transforms.ToTensor()
        ])
    return prep
...
class LuminanceRemapper():
    ...
    def shift(self, style_image: Image):
        ...
        # shift luminance of style image
...
def calc_mean_std(feat, eps=1e-6):
    """
    Calculates mean and std over height and width of image (batch and channel wise).
    """
    size = feat.size()
    assert (len(size) == 4)
    N, C = size[:2]
...
def resize_scalar_field(image, out_keys):
    out = {}
    out['r11'] = image
    out['r12'] = image 
...
def receptive_resize_scalar_field(image, out_keys, use_relu = False):
    # avg pool instead of conv
    inverse_conv = nn.Sequential(
        nn.ReflectionPad2d((1, 1, 1, 1)),
        nn.AvgPool2d((3, 3), stride=1)
    )
    max_pool = nn.MaxPool2d((2, 2), (2, 2), (0, 0), ceil_mode=True)
    #NOTE: for min pool instead of max pool
    # max_pool_2x2 = nn.MaxPool2d((2, 2), (2, 2), (0, 0), ceil_mode=True)
...
def minpool_resize_scalar_field(image, out_keys, use_relu = False):
    # min pool instead of conv
    max_pool_3x3 = nn.Sequential(
        nn.ReflectionPad2d((1, 1, 1, 1)),
        nn.MaxPool2d((3, 3), 1, (0, 0), ceil_mode=True)
    )
